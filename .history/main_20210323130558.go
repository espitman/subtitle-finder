...
func addDirsResult(dirName string, subName string) {
	style := fyne.TextStyle{
		Bold: true,
		Face: storage.LoadResourceFromURI("content://custom.ttf"),
		// no more "Monospace" required if "Face" were a thing
		Size: 24,
	}
	// content.Add(widget.NewLabel(dirName))
	content.Add(widget.NewLabelWithStyle(dirName, fyne.TextAlignLeading, fyne.TextStyle{Italic: true, Monospace: true}))
}
